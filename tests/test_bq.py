...
class BqCliTests(unittest.TestCase):
  def setUp(self) -> None:
    ...
    config_path.write_text(
      json.dumps({"definitionId": "definition-default", "concurrencyDefinitions": {}}),
      encoding="utf-8",
    )
  ...
  def test_internal_input_preserves_absent_and_present_empty_stdin(self) -> None:
    ...
    with patch("sys.stdin", io.StringIO(json.dumps({**required_input, "stdin": ""}))):
  ...
  def test_internal_input_accepts_valid_optional_environment_and_extra_field(
    ...
    self.assertEqual(loaded["ollamaEnvironment"], trigger_input["ollamaEnvironment"])
  ...
  def test_linux_session_environment_uses_only_a_private_owned_runtime(self) -> None:
    ...
    self.assertEqual(module["linux_session_environment"](runtime_path, os.getuid()), {})
