    ...
    def test_import_visit_save_and_restart_preserve_pending_and_running_windows(self):
        with tempfile.TemporaryDirectory(prefix="lazy-demo-test-") as temporary:
            ...
            try:
                ...
                self.assertEqual(
                    tmux(
                        "display-message", "-p", "-t", "alpha:2", "#{pane_current_path}"
                    ),
                    str(home / "two"),
                )
                ...
                try:
                    ...
                    while b"BEFORE-SWITCH" not in initial:
                        ...
                        if (
                            remaining <= 0
                            or not select.select([master], [], [], remaining)[0]
                        ):
                            break
                    ...
                    self.assertEqual(
                        [p.name for p in exported.iterdir()], ["state.json"]
                    )
                    self.assertEqual(
                        (exported / "state.json").read_bytes(), saved_bytes
                    )
                    self.assertEqual(
                        (exported / "state.json").stat().st_mode & 0o777, 0o600
                    )
                finally:
    ...
    def test_layout_round_trip_remaps_home_and_missing_cwd_never_partially_activates(
        ...
        with tempfile.TemporaryDirectory(prefix="lazy-layout-") as temporary:
            ...
            try:
                ...
                self.assertTrue(
                    saved["sessions"][0]["windows"][0]["panes"][1]["active"]
                )
