...
class CommitCommandTests(unittest.TestCase):
  def setUp(self) -> None:
    ...
    shutil.copy2(PROJECT_ROOT / "scripts" / "check_staged", scripts / "check_staged")
    self.run_git("init", "-q")
  ...
  def test_partially_staged_file_is_refused_without_changing_index(self) -> None:
    ...
    self.assertFalse((self.repo / "model.log").exists())
    self.assertEqual(index_before, self.run_git("diff", "--cached", "--binary").stdout)
  ...
  def test_success_checks_and_commits_only_the_staged_selection(self) -> None:
    ...
    self.assertEqual(
      "tracked.txt\n",
      self.run_git("diff-tree", "--no-commit-id", "--name-only", "-r", "HEAD").stdout,
    )
  ...
  def test_commit_succeeds_without_a_terminal(self) -> None:
    ...
    result = self.run_commit(extra_env={"TMUX": "", "TMUX_PANE": "", "TERM": "dumb"})
