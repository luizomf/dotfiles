...
def allow_cleanup(_root: Path) -> None:
    return None
...
class CleanupTests(unittest.TestCase):
    def setUp(self):
        ...
        self.run_dir = self.root / "pi.ephemeral-runs/run.ABC123"
        self.run_dir.mkdir(parents=True)
        (self.run_dir / ".auth").write_text("fixture")
    ...
    def test_unknown_names_and_history_are_not_cleanup_targets(self):
        unknown = self.run_dir.parent / "run.too-long"
        unknown.mkdir()
...
class StopTests(unittest.TestCase):
    def scenario(self, mode: str) -> tuple[int, str]:
