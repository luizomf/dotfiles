...
class SyncHostsTests(unittest.TestCase):
  def setUp(self):
    ...
    for host in ["m132", "m4128", "fedoraair", "extra"]:
      ...
      (home / "Desktop/tutoriais_e_cursos/project/.omnews-data/local.db").write_text(
        host
      )
  ...
  def test_new_files_from_each_peer_reach_every_host_in_one_run(self):
    ...
    for host in ["m132", "m4128", "fedoraair"]:
      ...
      self.assertEqual(
        (home / "Desktop/tutoriais_e_cursos/project/.omnews-data/local.db").read_text(),
        host,
      )
  ...
  def test_whole_private_roots_travel_with_only_shared_exclusions(self):
    ...
    for host in ["m132", "fedoraair"]:
      ...
      self.assertEqual((destination / link_path).readlink(), Path(relative_target))
  ...
  def test_auth_sync_keeps_machine_identity_and_absent_or_failed_auth(self):
    ...
    for host in ["m132", "m4128", "fedoraair"]:
      ...
      self.assertEqual((home / ".codex/auth.json").read_text(), "synthetic-" + expected)
      if host != "m132":
        self.assertEqual(
  ...
  def test_gio_fallback_uses_only_mocked_trash(self):
    ...
    self.script.write_text(
      source.replace("if command -v trash >/dev/null 2>&1; then", "if false; then")
    )
    result = self.run_sync()
    self.assertEqual(result.returncode, 0, result.stderr)
  ...
  def test_additional_host_does_not_trigger_idle_maintenance(self):
    ...
    relevant = [
      c
      for c in calls
      if "scripts/clear_sannux_transients" in c or "scripts/stop_omnivoicetts" in c
    ]
  ...
  def test_failed_prerequisites_and_pushes_report_and_continue_independent_work(self):
    ...
    self.assertTrue(
      (
        self.root / "homes/fedoraair/Desktop/tutoriais_e_cursos/project/m132.txt"
      ).exists()
    )
  ...
  def test_an_empty_additional_host_receives_the_collected_files(self):
    ...
    for origin in ["m132", "m4128", "fedoraair"]:
      source = (
        self.root / "homes/extra/Desktop/tutoriais_e_cursos/project" / (origin + ".txt")
      )
