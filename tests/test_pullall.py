...
class PullAllTests(unittest.TestCase):
  def setUp(self):
    ...
    timeout.write_text(
      ...
      "args = [{'60s': '0.5s', "
      "'--kill-after=5s': '--kill-after=0.2s'}.get(a, a) for a in "
      "sys.argv[1:]]\n"
    ...
    shim.write_text(
      ...
      "   (pathlib.Path(os.environ['HOME']) / (role + '.pid')).write_text("
      "str(os.getpid()))\n"
  ...
  def repository(self, path: Path) -> Path:
    ...
    # Safe: git is resolved once from PATH and all arguments are fixture paths.
  ...
  def run_pullall(self, **env: str) -> subprocess.CompletedProcess[str]:
    # Safe: fixed Bash and repository script, with an isolated fixture environment.
  ...
  def test_stuck_pull_and_child_are_killed_before_continuing(self):
    ...
    for role in ["parent", "child"]:
      ...
      # Safe: ps is fixed and the PID was generated by the isolated fixture.
