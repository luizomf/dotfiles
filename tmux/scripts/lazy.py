...
class LazyTmux:
        ...
        for session in sessions.values():
            ...
            for window in session["windows"]:
                for p in sorted(
                    ...
                    window["panes"].append(
                        {"cwd": cwd, "title": p[6], "active": p[8] == "1"}
                    )
        if not sessions:
    ...
    def restore_structure(self, state):
        ...
        for session in state["sessions"]:
            ...
            dimensions = re.match(
                r"^[0-9a-f]+,(\d+)x(\d+),", session["windows"][0]["layout"]
            )
            ...
            for number, window in enumerate(session["windows"]):
                if number == 0:
                    ...
                    if window["index"] != 1:
                        self.tmux(
                            "move-window", "-s", wid, "-t", f"{sid}:{window['index']}"
                        )
                else:
                    wid = self.tmux(
                        "new-window",
                ...
                self.tmux(
                    "select-layout", "-t", wid, window["layout"] or "even-horizontal"
                )
    ...
    def activate(self, window):
        ...
        for row, cwd in zip(rows, directories):
            if row[1] not in ("", "0"):
                raise RuntimeError(
                    "Marked pane already has a process; refusing to replace it"
                )
            self.tmux(
                "respawn-pane",
                "-t",
    ...
    def visit(self, window, session, generation):
        ...
        try:
            ...
            if (
                session
                not in self.tmux("list-sessions", "-F", "#{session_id}").splitlines()
            ):
                return
            if (
                self.tmux("display-message", "-p", "-t", session, "#{window_id}")
                != window
            ):
                return
    ...
    def status(self):
        ...
        return {
            "running": True,
            "sessions": len(
                self.tmux("list-sessions", "-F", "#{session_id}").splitlines()
            ),
            "windows": len(
...
def main():
    ...
    with app.locked():
        ...
        if args.action == "import":
            ...
            app.notify(
                "Tmux server stopped; only explicitly saved structure will return"
            )
