...
| `~/sannux-data/`                | Whole directory, including persistent homes, workspaces, worktrees, backups and private resources; shared exclusions and known auth rules still apply |
...
these directories inside the separately synchronized Pi/Sannux roots.
...
There are no blanket exclusions for Sannux sessions, workspaces, worktrees,
backup directories, installation metadata, or unlisted files. Git metadata and
the shared filename/path exclusions above still apply inside those directories.
